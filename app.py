...
if run_button and uploaded_file is not None:
    ...
    try:
        with st.status("Running pipeline...", expanded=True) as status:
            ...
            with st.expander("Score breakdown (debug)", expanded=True):
                st.dataframe(
                    df[["candidate_id", "skill_score", "semantic_score", "score"]],
                    use_container_width=True,
                    hide_index=True,
                )

            # SANDBOX-ONLY BEHAVIOR: the real submission (main.py) enforces the
            # skill/semantic gates strictly, as required by the spec, against
            # the full 100K-candidate pool. Here, in a small-sample demo, those
            # gates can eliminate everyone even though the pipeline ran
            # correctly — there just aren't enough candidates for the gates to
            # have anything to work with. Rather than return an empty result,
            # we fall back to ranking ALL candidates that passed Step 1's hard
            # filters by final score, and clearly label this as a fallback.
            if len(df) < TOP_N:
                if n_gated == 0:
                    st.info(
                        f"None of the {len(df)} candidates in this small sample "
                        f"cleared the skill/semantic score gates "
                        f"(skill_score > {MIN_SKILL_SCORE}, semantic_score > {MIN_SEMANTIC_SCORE}). "
                        "Since the sample is smaller than the top-100 target, the "
                        "sandbox falls back to ranking all candidates that passed "
                        "Step 1's hard filters by their composite score, ungated. "
                        "The full 100K-candidate submission enforces these gates "
                        "strictly — see main.py."
                    )
                else:
                    st.info(
                        f"{n_gated}/{len(df)} candidates cleared the score gates. "
                        "Since this sample is smaller than the top-100 target, the "
                        "sandbox ranks all Step-1-filtered candidates by score "
                        "(ungated) rather than dropping the rest. The full "
                        "100K-candidate submission enforces the gates strictly."
                    )
                df_final = df.copy()
            elif n_gated == 0:
                status.update(label="No candidates passed the score gates", state="error")
                st.warning(
                    f"{len(df)} candidates passed the hard filters in Step 1, but "
                    f"none cleared the skill/semantic score gates "
                    f"(skill_score > {MIN_SKILL_SCORE}, semantic_score > {MIN_SEMANTIC_SCORE})."
                )
                st.stop()
            elif n_gated < TOP_N:
