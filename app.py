...
if run_button and uploaded_file is not None:
    ...
    try:
        with st.status("Running pipeline...", expanded=True) as status:
            ...
            with st.expander("Score breakdown (debug)", expanded=(n_gated == 0)):
                st.dataframe(
            ...
            if n_gated == 0:
                ...
                st.warning(
                    ...
                    "This is common with very small samples — these thresholds were "
                    "tuned against the full 100K-candidate pool. Try uploading a "
                    "larger or more varied sample, or check the semantic_score "
                    "range printed above."
                )
                st.stop()
